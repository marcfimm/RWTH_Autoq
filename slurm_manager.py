...
class slurm_manager(object):
    ...
    def connect(self) -> None:
        ...
        if create_new_connection:
            ...
            # Hardcode remote_dir to a safe user-owned location
            self.remote_dir = f"/home/{self.user}/autoqchem/scratch"

            # Try to create the directory (warn=True avoids crashing if it already exists)
            self.connection.run(f"mkdir -p {self.remote_dir}", warn=True)
        ...
        if jobs:
            ...
            for name, job in jobs.items():
                ...
                with self.connection.cd(self.remote_dir):
                    ret = self.connection.run(f"/home/{self.user}/scripts/autosub16 {job.base_name}.gjf", hide=True)
    ...
    def _retrieve_single_job(self, job) -> slurm_status:
        ...
        try:  # try to fetch the file
            log_file = self.connection.get(f"{self.remote_dir}/{job.base_name}.log",     #used to be .gjf.out
    ...
    def remove_jobs(self, jobs) -> None:
        ...
        for name, job in jobs.items():
            ...
            self.connection.run(f'rm -f "{self.remote_dir}/{job.base_name}*"')#patched to accept the nonsensical zshell nonsense that CLAIX does
            del self.jobs[name]
        self._cache()
